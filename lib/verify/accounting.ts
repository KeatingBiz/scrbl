...
export function verifyAccounting(result: BoardUnderstanding): Verification | null {
  ...
  /* ========== 1) Accounting equation: A = L + E ========== */
  {
    ...
    if (A != null && L != null && E != null) {
      ...
      if (A == null && L != null && E != null && /\bassets?\b|\bA=/.test(finalS)) {
        pushCheck(checks, "assets", L + E, finalNum, 1e-2);
      } else if (L == null && A != null && E != null && /\bliabilit|debt\b|\bL=/.test(finalS)) {
        pushCheck(checks, "liabilities", A - E, finalNum, 1e-2);
      } else if (E == null && A != null && L != null && /\bequity\b|\bE=/.test(finalS)) {
  ...
  if (/\b(journal|entry|debit|credit|dr|cr)\b/.test(lower)) {
    // Grab all numbers near "debit"/"dr" and "credit"/"cr"
    const debits: number[] = [];
  ...
  if (/\b(inventory|cogs|fifo|lifo|weighted\s*average)\b/.test(lower)) {
    ...
    // Identity check if all four present
    if (BI != null && Purch != null && COGS != null && EI != null) {
      ...
      // Solve for whichever appears to be requested in the final string
      if (EI == null && BI != null && Purch != null && COGS != null && /\bending\s*inventory\b|\bEI\b/i.test(finalS)) {
        pushCheck(checks, "ending_inventory", BI + Purch - COGS, finalNum, 0.01);
      } else if (COGS == null && BI != null && Purch != null && EI != null && /\bCOGS\b|cost\s*of\s*goods\s*sold/i.test(finalS)) {
        pushCheck(checks, "COGS", BI + Purch - EI, finalNum, 0.01);
      } else if (BI == null && Purch != null && COGS != null && EI != null && /\bbeginning\s*inventory\b|\bBI\b/i.test(finalS)) {
  ...
  if (/\bdepreciation|straight\s*line|double\s*declining|ddb|units\s*of\s*production|uop\b/.test(lower)) {
    ...
    if (/\bstraight\s*line\b|\\bSL\\b/i.test(lower)) {
      if (cost != null && life != null) {
        ...
        if (finalNum != null && /\bdepre?c?i?a?t?i?o?n|expense|annual\b/i.test(finalS)) {
        ...
        if (year != null && year >= 1) {
          ...
          if (finalNum != null && /\bbook\s*value|BV\b/i.test(finalS)) {
    ...
    if (/\bdouble\s*declining|ddb\b/i.test(lower)) {
      if (cost != null && life != null) {
        ...
        let depY = 0;
        for (let i = 1; i <= y; i++) {
          const dep = Math.min(bvStart * rate, Math.max(0, (cost - salvage) - (cost - bvStart)));
          // Standard DDB with salvage floor at the end of life:
        ...
        if (finalNum != null && /\bbook\s*value|BV\b/i.test(finalS)) {
          pushCheck(checks, "book_value_DDB_y", book, finalNum, 0.01);
        }
      }
    }
    ...
    if (/\bunits\s*of\s*production|uop\b/i.test(lower)) {
      if (cost != null && totalUnits != null && unitsThis != null) {
        ...
        if (finalNum != null) {
          pushCheck(checks, "depreciation_UoP", dep, finalNum, 0.01);
        }
      }
    }
  }
  ...
  if (/\bratio|current\s*ratio|quick\s*ratio|acid[-\s]*test|debt[-\s]*to[-\s]*equity|gross\s*margin|roe|roa\b/.test(lower)) {
    ...
    // Quick ratio (cash + AR + other quick)/CL ; fallback ≈ (CA - Inventory - Prepaid)/CL
  ...
  if (/\bbreak[-\s]*even|contribution\s*margin|cvp|target\s*profit/i.test(lower)) {
    ...
    if (price != null && vcu != null && fixed != null) {
      const CM = price - vcu; // contribution margin per unit
      if (CM > 0) {
        // break-even units
