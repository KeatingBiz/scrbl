...
export function gatherProblemText(
  ...
  // collect step fields; they can be string | null
  const stepPieces = (steps ?? []).flatMap((s) => [s.before, s.after, s.text, s.action]);
...
export function extractNumberList(text: string): number[] {
  ...
  const nums = (segment.match(/-?\d*\.?\d+(?:e[+-]?\d+)?/gi) || []).map((n) => parseFloat(n));
